...
async def _user(client, email):
    await client.post(
        "/auth/register", json={"email": email, "name": email, "password": "pw12345"}
    )
...
async def test_create_org_makes_owner(client):
    ...
    resp = await client.post(
        "/orgs", json={"name": "ForgeAI Labs"}, headers=_auth(token)
    )
...
async def test_workspace_isolation_blocks_outsider(client):
    ...
    ws = (
        await client.post("/orgs", json={"name": "Labs"}, headers=_auth(owner))
    ).json()["workspace_id"]
...
async def test_invite_requires_admin(client):
    ...
    await client.post(
        "/orgs/invite/accept", json={"token": token}, headers=_auth(member_token)
    )
...
async def test_invite_accept_grants_membership_and_activity(client):
    ...
    assert (
        "org.created" in actions
        and "member.invited" in actions
        and "member.joined" in actions
    )
